...
contract ZionTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SZION }

    address public immutable ZION;
    ...
    address public sZION;
    uint public sZIONQueue; // Delays change to sZION address
    ...
    /**
        @notice allow approved address to deposit an asset for ZION
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint ZION needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( ZION ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn ZION for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IZIONERC20( ZION ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sZION ).balanceOf( msg.sender ); // Can only borrow against sZION held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with ZION
        @param _amount uint
     */
    function repayDebtWithZION( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IZIONERC20( ZION ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, ZION, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( ZION ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( ZION ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns ZION valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match ZION decimals
            value_ = _amount.mul( 10 ** IERC20( ZION ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SZION ) { // 9
            sZIONQueue = block.number.add( blocksNeededForQueue );
        } else return false;
