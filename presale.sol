...
interface IPresaleZion {
    function mint(address account_, uint256 amount_) external;
}

contract ZionPresale is Ownable {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    struct UserInfo {
        uint256 amount; // Amount BUSD deposited by user
        uint256 debt; // total ZION claimed thus pZION debt
        bool claimed; // True if a user has claimed ZION
    }

    address devAddress = 0xE085844c422ad0fF346206faf70e3CA0c806517E;
    address public homeAddress = 0x90600F6f1066f940A64A0259393C409e8308822F; 

    // Tokens to raise (BUSD) and for offer (pZION) which can be swapped for (ZION)
    IERC20 public BUSD; // for user deposits
    IERC20 public pZION;
    IERC20 public ZION;

    uint256 public price = 20 * 1e18; // 20 BUSD per pZION

    uint256 public cap = 1000 * 1e18; // 1000 BUSD cap per user

    uint256 public totalRaisedBUSD; // total BUSD raised by sale

    uint256 public totalDebt; // total pZION and thus ZION owed to users
    ...
    bool public claimPresale; // true when pZION is claimable

    bool public contractPaused; // circuit breaker

    mapping(address => UserInfo) public userInfo;
    mapping(address => uint256) public everClaimable; // amount of ZION claimable by address
    ...
    constructor(
        ...
        require( _pZION != address(0) );
        pZION = IERC20(_pZION);
        require( _ZION != address(0) );
        ZION = IERC20(_ZION);
    ...
    /**
     *  @notice set ZION Token
     *  @param _ZION: address of ZION
     */
    function setZION(address _ZION) external onlyOwner {
        require( _ZION != address(0) );
        ZION = IERC20(_ZION);
    }

    /**
     *  @notice set pZION Token
     *  @param _pZION: address of pZION
     */
    function setPZION(address _pZION) external onlyOwner {
        require( _pZION != address(0) );
        pZION = IERC20(_pZION);
    }

    /**
     *  @notice set price for pZION
     *  @param _price: price for pZION
     */
    ...
    // @notice lets users claim ZION
    // @dev send sufficient ZION before calling
    function claimUnlock() external onlyOwner {
        require(ended, "Sale has not ended");
        require(!claimable, "Claim has already been unlocked");
        require(ZION.balanceOf(address(this)) >= totalDebt, 'not enough ZION in contract');
        claimable = true;
        emit ClaimUnlocked(block.number);
    }


    // @notice lets users claim pZION
    ...
    function deposit(uint256 _amount) external checkIfPaused {
        ...
        uint256 payout = _amount.mul(1e18).div(price).div(1e9); // pZION to mint for _amount
        ...
        IPresaleZion( address(pZION) ).mint( msg.sender, payout );

        emit Deposit(msg.sender, _amount);
    }
    
    /**
     *  @notice it deposits pZION to withdraw ZION from the sale
     *  @param _amount: amount of pZION to deposit to sale (9 decimals)
     */
    function withdraw(uint256 _amount) external checkIfPaused {
        require(claimable, 'ZION is not yet claimable');
        ...
        pZION.safeTransferFrom( msg.sender, address(this), _amount );

        ZION.safeTransfer( msg.sender, _amount );

        emit Mint(address(pZION), msg.sender, _amount);
        emit Withdraw(address(ZION), msg.sender, _amount);
    }
    ...
    // @notice it claims pZION back from the sale
    function claimPresaleZION() external checkIfPaused {
        require(claimPresale, 'pZION is not yet claimable');
        ...
        pZION.safeTransfer( msg.sender, payout );

        emit Withdraw(address(pZION),msg.sender, payout);
    }
